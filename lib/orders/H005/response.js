...
module.exports = (xml, keys) => ({
	...
	orderData() {
		const orderDataNode = this.doc.getElementsByTagNameNS('urn:org:ebics:H005', 'OrderData');
	...
	transactionKey() {
		const keyNodeText = this.doc.getElementsByTagNameNS('urn:org:ebics:H005', 'TransactionKey')[0].textContent;
	...
	businessSymbol(code) {
		return errors.business[code] ? errors.business[code].symbol : '';
	},

	businessShortText(code) {
		return errors.business[code] ? errors.business[code].short_text : '';
	},

	businessMeaning(code) {
		return errors.business[code] ? errors.business[code].meaning : '';
	},
	...
	technicalShortText(code) {
		return errors.technical[code] ? errors.technical[code].short_text : '';
	},

	technicalMeaning(code) {
		return errors.technical[code] ? errors.technical[code].meaning : '';
	},
